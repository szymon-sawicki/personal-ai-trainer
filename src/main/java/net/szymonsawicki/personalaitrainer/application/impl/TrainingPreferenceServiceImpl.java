...
public class TrainingPreferenceServiceImpl implements TrainingPreferenceService {
    ...
    public TrainingPreferenceDto createTrainingPreference(TrainingPreferenceCreateDto preferenceDto) {
        ...
        Person person = personRepository.findById(preferenceDto.personId())
                .orElseThrow(() -> new EntityNotFoundException("Person not found with id: " + preferenceDto.personId()));
    ...
    @Override
    @Transactional
    public TrainingPreferenceDto updateTrainingPreference(Long id, TrainingPreferenceCreateDto preferenceDto) {
        TrainingPreference preference = trainingPreferenceRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Training preference not found with id: " + id));
    ...
    public TrainingPreferenceDto getTrainingPreferenceByPersonId(Long personId) {
        TrainingPreference preference = trainingPreferenceRepository.findByPersonId(personId)
                .orElseThrow(() -> new EntityNotFoundException("Training preference not found for person id: " + personId));
        return trainingPreferenceMapper.toDto(preference);
    }
